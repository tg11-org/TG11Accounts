...
"""End-to-end TG11 sign-in smoke test for Echoquill, run on the server.
...
Usage:  python smoke_echoquill.py <tg11-username> <password>
...
def run_flow(client: httpx.Client, label: str) -> httpx.Response:
    ...
    check(f"{label}: landed signed in on Echoquill", resp.url.host == "echoquill.tg11.org"
...
def main() -> int:
    ...
    with httpx.Client(follow_redirects=False, timeout=30) as client:
        if not idp_login(client, username, password):
            return 1
        first = run_flow(client, "first login")
        check("first login shows the TG11 username", username in first.text or "profile" in str(first.url))

        # local logout, then straight back in
        page = client.get(f"{APP}/accounts/profile/", follow_redirects=True)
        m = re.search(r'name="csrfmiddlewaretoken"\s+value="([^"]+)"', page.text)
        if m:
            client.post(f"{APP}/accounts/logout/", data={"csrfmiddlewaretoken": m.group(1)},
                        headers={"Referer": f"{APP}/accounts/profile/"}, follow_redirects=True)
        after = client.get(f"{APP}/accounts/profile/", follow_redirects=False)
